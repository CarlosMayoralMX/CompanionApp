...
public final class Kumulos {
    ...
    private static boolean lifecycleWatcherInitialised = false;
    ...
    public static synchronized void initialize(final Application application, KumulosConfig config) {
        ...
        maybeRegisterLifecycleWatcher(application);
    ...
    //Kompanion specific
    public static void maybeRegisterLifecycleWatcher(final Application application){
        if (!lifecycleWatcherInitialised){
            application.registerActivityLifecycleCallbacks(new AnalyticsContract.ForegroundStateWatcher(application));
            lifecycleWatcherInitialised = true;
        }
    }
